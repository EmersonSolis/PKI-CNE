...
function approveForMyOrg() {
  ...
  if [ $ORG -eq 1 ]; then
    peer lifecycle chaincode approveformyorg -o localhost:7050 --ordererTLSHostnameOverride orderer.cne.com --tls --cafile "$ORDERER_CA" --channelID $CHANNEL_NAME --name ${CC_NAME} --version ${CC_VERSION} --package-id ${PACKAGE_ID} --sequence ${CC_SEQUENCE} ${INIT_REQUIRED} ${CC_END_POLICY} ${CC_COLL_CONFIG} >&log.txt
  elif [ $ORG -eq 2 ]; then
    peer lifecycle chaincode approveformyorg -o localhost:8050 --ordererTLSHostnameOverride orderer.moe.com --tls --cafile "$ORDERER_CA" --channelID $CHANNEL_NAME --name ${CC_NAME} --version ${CC_VERSION} --package-id ${PACKAGE_ID} --sequence ${CC_SEQUENCE} ${INIT_REQUIRED} ${CC_END_POLICY} ${CC_COLL_CONFIG} >&log.txt
  fi
...
function commitChaincodeDefinition() {
  ...
  if [ $ORG -eq 1 ]; then
    peer lifecycle chaincode commit -o localhost:7050 --ordererTLSHostnameOverride orderer.cne.com --tls --cafile "$ORDERER_CA" --channelID $CHANNEL_NAME --name ${CC_NAME} "${PEER_CONN_PARMS[@]}" --version ${CC_VERSION} --sequence ${CC_SEQUENCE} ${INIT_REQUIRED} ${CC_END_POLICY} ${CC_COLL_CONFIG} >&log.txt
  elif [ $ORG -eq 2 ]; then
    peer lifecycle chaincode commit -o localhost:8050 --ordererTLSHostnameOverride orderer.moe.com --tls --cafile "$ORDERER_CA" --channelID $CHANNEL_NAME --name ${CC_NAME} "${PEER_CONN_PARMS[@]}" --version ${CC_VERSION} --sequence ${CC_SEQUENCE} ${INIT_REQUIRED} ${CC_END_POLICY} ${CC_COLL_CONFIG} >&log.txt
  fi
...
function chaincodeInvokeInit() {
  ...
  if [ $ORG -eq 1 ]; then
    peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.cne.com --tls --cafile "$ORDERER_CA" -C $CHANNEL_NAME -n ${CC_NAME} "${PEER_CONN_PARMS[@]}" --isInit -c ${fcn_call} >&log.txt
  elif [ $ORG -eq 2 ]; then
    peer chaincode invoke -o localhost:8050 --ordererTLSHostnameOverride orderer.moe.com --tls --cafile "$ORDERER_CA" -C $CHANNEL_NAME -n ${CC_NAME} "${PEER_CONN_PARMS[@]}" --isInit -c ${fcn_call} >&log.txt
  fi
